...
public class GoldenTests
{
    ...
    public void GoldenScans_AreByteIdentical()
    {
        ...
        string candlesOnly = File.ReadAllText(Path.Combine(golden, "data.json"));
        string fed = File.ReadAllText(Path.Combine(golden, "data-feeds.json"));
        using JsonDocument candleData = JsonDocument.Parse(candlesOnly);
        using JsonDocument fedData = JsonDocument.Parse(fed);
        ...
        foreach (string specPath in Directory.GetFiles(Path.Combine(golden!, "specs"), "*.json"))
        {
            ...
            JsonDocument data = name.StartsWith("feeds_", StringComparison.Ordinal)
                ? fedData
                : candleData;
