## Golden-corpus cases for the wickra-screener R binding.
##
...
## exist, and that is what r-universe runs on every platform.
##
## The parity block below degrades to a skip when the corpus is absent. The
## streaming block deliberately does not: its comment argues that a missing
## corpus is a broken checkout rather than a phase that has not arrived, which
## is true in a repository and false in a tarball. That stop is what took
## wickrascreener 0.1.2 to 0 of 13 platform binaries with
##
##     Error: golden fixtures not found; the streaming comparison would test nothing
##
## Both belong here rather than one of them being softened: a shipped test
## should not reason about the repository at all.
...
cat("wickra-screener R golden cases passed\n")
